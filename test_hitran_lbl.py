...
if (__name__ == "__main__"):
    ...
    for iso in range(Nbiso):
        ...
        sigma2[iso,:] = pytran.calculate_hitran_xsec(linelist, wn, T=150., P=7., Pref=1013.)

    fig, ax = plt.subplots()
    ax.plot(wn, sigma2.sum(axis=0), 'k', alpha=0.5)
    for iso in range(Nbiso):
        ax.plot(wn, sigma2[iso,:], label=pytran.get_iso_name(5,iso+1))
    ax.set_yscale('log')
    ax.legend()
    ax.set_xlabel(r'$\nu$ (cm$^{-1}$)')
    ax.set_ylabel(r'(c$^2$)')
